...
namespace FirstMenu
{
    public partial class Deposit : Form
    {
        ...
        public Deposit(String databaseFile, Account selectedAccount)
        {
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            if (this.ValidateChildren())
            {
                ...
                OleDbCommand deposit = new OleDbCommand();
                deposit.Connection = bankingDatabaseConnection;
                deposit.CommandText =
                    "SELECT * from Banking where [Routing Number]=" + int.Parse(account.RoutingNum) + "";
                OleDbDataReader reader = deposit.ExecuteReader();

                int count = 0;
                double balance = 0;
                string replace = textBox1.Text.Replace(",", "").Replace("$", "").TrimStart('0'); //removes currency formating
                double amountAdded = double.Parse(replace); //converts to double
                string transaction = "";
                ...
                balance = balance + amountAdded; //adds amount to balance

                //date, from, transaction, amount, balance;
                transaction += 
                    DateTime.Now.ToString("f") + 
                    "|Deposit" + 
                    "|Deposit from Tylanni Banking Software|" + 
                    textBox1.Text + "|" +
                    balance.ToString("C") + ";"; 
                ...
                deposit.ExecuteNonQuery(); //updates balance
